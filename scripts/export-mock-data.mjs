...
const sourcePath = resolve(projectRoot, "src/data.ts");
const fighterRaidSourcePath = resolve(projectRoot, "src/generated/fighterRaidMixed50.ts");
const tempDir = resolve(projectRoot, ".mock-export");
const tempModulePath = resolve(tempDir, "data.mjs");
const tempFighterRaidModulePath = resolve(tempDir, "generated/fighterRaidMixed50.mjs");
const outputDir = resolve(projectRoot, "data/export");
...
const transpiled = ts
  .transpileModule(source, transpileOptions)
  .outputText.replace(/from "\.\/generated\/fighterRaidMixed50";/g, 'from "./generated/fighterRaidMixed50.mjs";');
const fighterRaidTranspiled = ts.transpileModule(await readFile(fighterRaidSourcePath, "utf8"), transpileOptions).outputText;

await rm(tempDir, { recursive: true, force: true });
await mkdir(tempDir, { recursive: true });
await mkdir(dirname(tempFighterRaidModulePath), { recursive: true });
await mkdir(outputDir, { recursive: true });
await writeFile(tempModulePath, transpiled, "utf8");
await writeFile(tempFighterRaidModulePath, fighterRaidTranspiled, "utf8");

const dataModule = await import(`${pathToFileURL(tempModulePath).href}?t=${Date.now()}`);
const {
  catalogRefs,
...
const toHandoffMessage = (message) => {
  const { provenanceDetail, ...rest } = message;
  return {
    ...rest,
    provenance: provenanceDetail ?? {
      sourceKind: "SYNTHETIC_LINK16_LIKE",
      sourceIds: Array.isArray(message.provenance) ? message.provenance : [],
      catalogSourceIds: Array.isArray(message.provenance) ? message.provenance : [],
      usageNote: "Synthetic fallback provenance generated during export.",
    },
  };
};

const toHandoffPreset = (preset) => ({
  ...preset,
  messages: preset.messages.map(toHandoffMessage),
});
...
const allInOne = {
  ...
  catalogRefs,
  publicMilitaryCatalog,
  simulationPresets: simulationPresets.map(toHandoffPreset),
};

const presetIndex = simulationPresets.map((preset) => ({
  id: preset.id,
  title: preset.title,
  shortTitle: preset.shortTitle,
  severity: preset.severity,
  regionLabel: preset.regionLabel,
  messageCount: preset.messages.length,
  assetCount: preset.assets.length,
  entityCount: preset.entities?.length ?? 0,
  trajectoryCount: preset.trajectorySpecs?.length ?? 0,
  decisionRecordCount: preset.decisionRecords?.length ?? 0,
  eventLogCount: preset.eventLog?.length ?? 0,
  threatClasses: [...new Set(preset.messages.map((message) => message.threatClassId).filter(Boolean))],
  highlights: preset.highlights,
}));

const readme = `# Link-16 COP Mock Data
...
await writeFile(resolve(outputDir, "link16-cop-simulation.v2.mock.json"), `${JSON.stringify(handoffPayload, null, 2)}\n`, "utf8");
await writeFile(resolve(outputDir, "link16-cop-mock-data.json"), `${JSON.stringify(allInOne, null, 2)}\n`, "utf8");
await writeFile(
  resolve(outputDir, "simulation-presets.mock.json"),
  `${JSON.stringify({ metadata, defaultSimulationId, simulationPresets: allInOne.simulationPresets }, null, 2)}\n`,
  "utf8",
);
await writeFile(
  resolve(outputDir, "osint-sources.mock.json"),
  `${JSON.stringify({ metadata, osintSources }, null, 2)}\n`,
  "utf8",
);
await writeFile(
  resolve(outputDir, "simulation-index.mock.json"),
  `${JSON.stringify({ metadata, defaultSimulationId, presets: presetIndex }, null, 2)}\n`,
  "utf8",
);
await writeFile(resolve(outputDir, "README.md"), readme, "utf8");
await rm(tempDir, { recursive: true, force: true });

console.log(`Exported ${simulationPresets.length} simulation presets to ${outputDir}`);
